    ...
    const overlapping = await db
      ...
      .where(
        and(
          ...
          or(
            and(
              lte(schema.projectAllocation.start_date, startDateStr),
              gte(schema.projectAllocation.end_date, startDateStr),
            ),
            endDateStr
              ? and(
                  lte(schema.projectAllocation.start_date, endDateStr),
                  gte(schema.projectAllocation.end_date, endDateStr),
                )
              : sql`false`,
            endDateStr
              ? and(
                  gte(schema.projectAllocation.start_date, startDateStr),
                  lte(schema.projectAllocation.end_date, endDateStr),
                )
              : sql`false`,
          ),
        ),
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (
      ...
      const newEndDateStr = end_date
        ? toDateString(end_date)
          ...
          and(
            ...
            or(
              and(
                ...
                gte(
                  schema.projectAllocation.end_date,
                  currentAllocation.start_date,
                ),
              ),
              newEndDateStr
